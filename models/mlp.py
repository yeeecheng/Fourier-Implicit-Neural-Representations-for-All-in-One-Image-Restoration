    ...
    def query_rgb(self, inp, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # print(f'q_coord : {q_coord.shape}, points_enc : {points_enc.shape}')
                ...
                # print(q_feat.shape, rel_coord.shape, inp.shape)
                if self.cell_decode:
                    rel_cell = cell.clone()
                ...
                preds.append(pred)
                # print(f'input : {inp.view(bs * q, -1).shape}, output : {pred.shape}')
...
if __name__ == "__main__":
    dim = 32
    img = torch.randn((16, dim, 64, 64)).to('cuda')
    inr = INR(dim).to('cuda')
    out = inr(img)
    print(out.shape)